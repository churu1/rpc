                            ...
                            google::protobuf::Closure *done)
{
  ...
  if (my_controller == NULL) {
    ERRORLOG("failed callmethod, RpcController covert error");
    return;
  }
  ...
  if (!m_is_init) {
    ...
    ERRORLOG("%s | %s, RpcChannel not init", req_protocol->m_msg_id.c_str(), err_info.c_str(), err_info.c_str());
    return;
  }
  ...
  m_client->connect([ req_protocol, channel]() mutable {
    ...
    if (channel->getTcpClient()->getConnectErrorCode() != 0) {
      my_controller->SetError(channel->getTcpClient()->getConnectErrorCode(), channel->getTcpClient()->getErrorInfo());
      ERRORLOG("%s | connect error, error code[%d], error info[%s], peer addr[%s]", req_protocol->m_msg_id.c_str(), my_controller->GetErrorCode(), 
        my_controller->GetErrorInfo().c_str(), channel->getTcpClient()->getPeerAddr()->toString().c_str());
      return;
    }

    channel->getTcpClient()->writeMessage(req_protocol, [req_protocol, channel, my_controller](AbstractProtocol::s_ptr msg) mutable {
      ...
      channel->getTcpClient()->readMessage(req_protocol->m_msg_id, [channel, my_controller] (AbstractProtocol::s_ptr msg) mutable {
        ...
        INFOLOG("%s | success get response, call method name[%s], peer addr[%s], local addr[%s]", 
        ...
        if (rsp_protocol->m_err_code != 0) {
          ERRORLOG("%s | call method[%s] failed, error code[%d], error info[%s]",
            rsp_protocol->m_msg_id.c_str(), rsp_protocol->m_method_name.c_str(),
