...
TcpAcceptor::TcpAcceptor(NetAddr::s_ptr local_addr) : m_local_addr(local_addr){
  ...
  if (setsockopt(m_listenfd, SOL_SOCKET, SO_REUSEADDR, &val, sizeof(val)) != 0) {
    ERRORLOG("setsocketopt REUSEADDR error, errno=[%d], error info=[%s]", errno, strerror(errno));
  }
...
int TcpAcceptor::accpet() {
  if (m_family == AF_INET) {
    ...
    if (client_fd < 0) {
      ERRORLOG("accept error, errno=[%d], error info=[%s]", errno, strerror(errno));
      exit(0);
    }
